"""
Mini grid search resserrée — recherche d'hyperparamètres autour de la meilleure configuration.

Usage:
    python -m src.refined_grid_search --config configs/config.yaml --epochs 5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Mini grid search resserrée")
    parser.add_argument('--config', type=str, default='configs/config.yaml',
                       help='Chemin vers config.yaml')
    parser.add_argument('--epochs', type=int, default=5,
                       help='Nombre d\'époques par run (défaut: 5)')
    ...
    # Vérifier que hparams_refined existe
    if 'hparams_refined' not in config:
        print("⚠️  Erreur: 'hparams_refined' non trouvé dans la config.")
        print("   Assurez-vous d'avoir ajouté la section hparams_refined dans config.yaml")
        return
    
    print("="*60)
    print("REFINED GRID SEARCH (Section 2.8)")
    print("="*60)
    ...
    # Lire les hyperparamètres de la grille resserrée
    hparams = config['hparams_refined']
    lrs = hparams['lr']
    ...
    print(f"\nGrille d'hyperparamètres (resserrée):")
    print(f"  - LR: {lrs}")
    ...
    checkpoint_file = os.path.join(artifacts_dir, 'refined_grid_search_checkpoint.json')
    ...
    print(f"\n{'='*60}")
    print(f"EXÉCUTION DE LA REFINED GRID SEARCH ({total_combinations} runs)")
    print(f"{'='*60}")
    ...
    for run_idx, (lr, weight_decay, dilation_stage3, blocks_per_stage) in enumerate(combinations):
        ...
        # Nom du run (avec préfixe refined_grid_)
        run_name = f"refined_grid_lr={lr:.4f}_wd={weight_decay_float:.0e}_dil={dilation_stage3}_blk={blocks_per_stage}"
    ...
    csv_path = os.path.join(artifacts_dir, 'refined_grid_search_results.csv')
    ...
    print(f"\n{'='*60}")
    print("MEILLEURE COMBINAISON (REFINED)")
    print(f"{'='*60}")
    ...
    # Heatmap: LR vs Weight Decay
    pivot_lr_wd = df.groupby(['lr', 'weight_decay'])['best_val_acc'].mean().unstack()
    im1 = axes[0].imshow(pivot_lr_wd.values, aspect='auto', cmap='viridis')
    axes[0].set_xticks(range(len(pivot_lr_wd.columns)))
    axes[0].set_xticklabels([f'{wd:.0e}' for wd in pivot_lr_wd.columns])
    axes[0].set_yticks(range(len(pivot_lr_wd.index)))
    axes[0].set_yticklabels([f'{lr:.4f}' for lr in pivot_lr_wd.index])
    axes[0].set_xlabel('Weight Decay', fontsize=12)
    axes[0].set_ylabel('Learning Rate', fontsize=12)
    axes[0].set_title('Val Accuracy: LR vs Weight Decay (Refined)', fontsize=14, fontweight='bold')
    plt.colorbar(im1, ax=axes[0])
    
    # Graphique: Val Accuracy par LR
    axes[1].boxplot([df[df['lr'] == lr]['best_val_acc'].values for lr in sorted(df['lr'].unique())], 
    ...
    axes[1].set_title('Distribution Val Accuracy par LR (Refined)', fontsize=14, fontweight='bold')
    axes[1].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plot_path1 = os.path.join(artifacts_dir, 'refined_grid_search_analysis.png')
    ...
    top_n = min(9, len(df_sorted))  # 9 combinaisons max
    ...
    plt.title(f'Top {top_n} Combinaisons - Val Accuracy (Refined Grid Search)', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3, axis='x')
    plt.tight_layout()
    
    plot_path2 = os.path.join(artifacts_dir, 'refined_grid_search_top_combinations.png')
    ...
    print("RÉSUMÉ")
    print(f"{'='*60}")
    print(f"✓ Refined grid search terminée")
    ...
    print(f"  - Graphiques: {artifacts_dir}/refined_grid_search_*.png")
    print(f"  - Logs TensorBoard: {runs_dir}/refined_grid_*")
